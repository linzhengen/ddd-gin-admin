...
type User interface {
	Query(ctx context.Context, req request.UserQuery) (*entity.Users, error)
	QueryShow(ctx context.Context, req request.UserQuery) (*response.UserShowQuery, error)
...
func (a *user) checkUserName(ctx context.Context, item schema.User) error {
	if item.UserName == schema.GetRootUser().UserName {
		return errors2.New400Response("The user name is invalid")
	}
	...
	if result.PageResult.Total > 0 {
		return errors2.New400Response("The user name already exists")
	}
	return nil
}
